...
TEST_CASE("jpeg")
{
...

    {
        auto output = ImageOutput::open(
            "test.jpg", {.width = img.w, .height = img.h, .component_type = ComponentType::U8, .component_count = 3});
        REQUIRE(output);
        CHECK(output->write_image(img.pixels.get(), img.w * img.h * sizeof(rgb8)));
    }

    {
        ...
        CHECK(input->read_image(img2.pixels.get(), img2.w * img2.h * sizeof(rgb8)));
        ...
        auto output = ImageOutput::open(
            "test2.jpg",
            {.width = img2.w, .height = img2.h, .component_type = ComponentType::U8, .component_count = 3});
        REQUIRE(output);
        CHECK(output->write_image(img2.pixels.get(), img2.w * img2.h * sizeof(rgb8)));
    }
}
